...
{
    [AddComponentMenu("")]
    class UnitySocialEntryPointController : MonoBehaviour
    {
        ...
        void OnEnable() //Initialization happens on OnEnable so the button can be easily hidden completely by setting the gameObject as inactive
        {
            ...
            {
                SocialOverlay.onEntryPointStateUpdated.AddListener(HandleUnitySocialEntryPointStateUpdate);

                SocialOverlay.notificationActorLocation = notificationLocation;
                SocialOverlay.notificationActorOffset = notificationOffset;
                SocialOverlay.entryPointUpdatesEnabled = true;
                SocialOverlay.entryPointImageSize = profileImageSize; //Configure the image size that the server returns
        ...
        void OnDisable()
        {
            ...
            {
                SocialOverlay.onEntryPointStateUpdated.RemoveListener(HandleUnitySocialEntryPointStateUpdate);
                SocialOverlay.notificationActorLocation = SocialOverlay.NotificationLocation.Hidden;
                SocialOverlay.entryPointUpdatesEnabled = false; //Do not fire notification events when the Entry Point is disabled
        ...
        {
            #if UNITY_SOCIAL
            SocialOverlay.EntryPointClicked();
            #endif
        }
