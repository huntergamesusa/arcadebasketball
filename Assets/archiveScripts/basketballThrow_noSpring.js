...
//	configurableJoint.spring = spring;
//	configurableJoint.damper = damper;
//	configurableJoint.maxDistance = distance;
	configurableJoint.connectedBody = ragdoll[1];
...
{
	var oldDrag = configurableJoint.connectedBody.drag;
	var oldAngularDrag = configurableJoint.connectedBody.angularDrag;
	configurableJoint.connectedBody.drag = drag;
	configurableJoint.connectedBody.angularDrag = angularDrag;
	var mainCamera = FindCamera();
	var rayz:float;
...
//rayy=Mathf.Clamp(rayy,460,1000);
		...
		//configurableJoint.transform.position

		configurableJoint.transform.position = ray.GetPoint(distance);
		

//configurableJoint.transform.position=Vector3(ray.GetPoint(distance).x,ray.GetPoint(distance).y,rayz);
...
yield;
		...
		//input.text=mousey.ToString();
	      ...
	      configurableJoint.connectedBody.AddForce(0,mousey,0);
		      configurableJoint.connectedBody.AddForce(0,0,mousez);
		      print("yForce: "+mousey+" , "+"zForce: "+mousez);
		       configurableJoint.connectedBody.AddForce(mousex*forceMult,0,0);
//		       configurableJoint.connectedBody.AddTorque(300,0,0);
		   // return;
//		print("mouseZ"+mousez);
		    configurableJoint.connectedBody.drag = oldDrag;
		   configurableJoint.connectedBody.angularDrag = oldAngularDrag;
		  ...
		  GetComponent.<basketballThrow_noSpring>().enabled = false;
